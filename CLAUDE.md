...
上記とは別に、**この API 自身も MCP サーバを公開する**（`/mcp`。`mcp` アダプタ）。これは開発者が自分の世界（セーブデータ）を覗くためのローカル探索ツールで、**`local` プロファイル限定**（既定は `spring.ai.mcp.server.enabled: false`）。`bootRun` は自動で `local` になるが、操作主体の解決に `MCP_SUBJECT_ID`（自分の Identity Platform の `sub`）を環境変数で渡す必要がある（[ADR-0072](docs/adr/0072-mcp-adapter-local-only-world-scoped.md)）。
...
- **`permitAll` は運用・CI が壊れるエンドポイントに限る**: `/actuator/health`（Cloud Run のヘルスチェック）、`/v3/api-docs` 配下と Swagger UI（`generateOpenApiDocs` が forked bootRun 経由で取得するため、認証を掛けると OpenAPI lint のゲートが壊れる）、MCP エンドポイント（**`local` プロファイル限定**。既定では `spring.ai.mcp.server.enabled: false` で口ごと開かない。[ADR-0072](docs/adr/0072-mcp-adapter-local-only-world-scoped.md)）。それ以外は `authenticated`
