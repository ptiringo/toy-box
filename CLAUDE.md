...
ヘルスチェックは `/actuator/health` で公開（公開設定は `application.yml`、詳細表示は認可時のみ）。`curl http://localhost:8080/actuator/health` で `{"status":"UP"}` が返る。アクセス方法・レスポンス形の正は `HealthEndpointTest.kt` を参照。
