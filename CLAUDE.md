...
issue / PR 操作には **GitHub MCP を使わず、`gh` CLI を直接使う**。

- 当初は sandbox 下の `gh` が TLS（`OSStatus -26276`）で詰まる問題の回避策として GitHub MCP を導入していたが、その GitHub MCP（`api.githubcopilot.com`）は **OAuth 認可サーバーが Dynamic Client Registration (DCR / RFC 7591) 非対応**で Claude Code の MCP OAuth フローが `Incompatible auth server: does not support dynamic client registration` で失敗し、PAT 方式に戻すと env 補間（`fnox exec`）の手間が増える。導入動機（gh の TLS 回避）が解消できる以上、MCP を持たない方がシンプルなので撤去した。
- sandbox 下の TLS 問題は、**`gh` を `.claude/settings.local.json` の `sandbox.excludedCommands` に登録**して sandbox の外で実行することで回避する（`"gh"` と `"gh *"` の両方を登録。マッチングは完全一致 + グロブのため。複合コマンド `A && gh ...` はマッチしないので `gh` は単体コマンドで実行する）。これにより `gh` は Seatbelt の TLS 検証問題（Go 製 CLI が `com.apple.trustd.agent` に届かず `OSStatus -26276` で失敗）を踏まない。
  - 別解として `sandbox.enableWeakerNetworkIsolation: true`（sandbox 内に `trustd` アクセスを開く）でも `gh` を sandbox 内に留めたまま TLS を通せるが、その分ネットワーク分離が弱まる。MITM プロキシ + 独自 CA でプロキシ経由を維持したい場合向けの選択肢であり、本リポジトリのように標準プロキシのみの環境では公式も `excludedCommands` を第一推奨としているため、こちらを採用している。
- これにより GitHub 操作は通常の `gh` コマンド（`gh pr ...` / `gh issue ...` 等）で完結する。env 補間も MCP 承認フローも不要。
...
- **現状、定義しているシークレットはない**（`fnox.toml` の `[secrets]` は空）。GitHub 操作は MCP ではなく `gh` CLI で行う方針のため PAT を fnox で管理する必要はない（上記「MCP サーバー設定」参照）。本仕組みは将来のシークレット（GCP 認証情報など）に備えて維持する
