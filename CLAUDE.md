...
- **PR のマージ方式**: 必ず **merge commit**（`gh pr merge --merge`）を使う。squash / rebase は使わない（個々のコミット履歴を main に残す方針）。**`--admin` は付けない**。main の ruleset が required status check を要求しており、`--admin` はそれを迂回する。チェックが緑になるのを待ってからマージする（[ADR-0083](docs/adr/0083-merge-without-admin-bypass.md)）。
