...
- **生成口の封じ込め**: コンストラクタは `private` にし、生成は companion object のファクトリ経由に限る。生成口はいずれも `public` とし、**集約をまたぐ前提条件**を持つもの（`BloodHorse.of` 等）は、前提を検証するドメインサービス（`service/` の対応領域）からのみ呼べるよう **ArchUnit ルールで封じ込める**（例: `BloodHorse.of` を `registerInStudBook` に封じ込め、`service.horse` 以外からの呼び出しを違反とする）。不変条件が**集約内で完結**するファクトリ（例: `Jockey.create`）は封じ込め対象外で、どこからでも呼べる。封じ込めのテストは Object Mother（`〜Fixture`）を `src/test` に置き、対象集約と同一パッケージに同居させる（ArchUnit はテストを除外するため Fixture からの呼び出しは縛られない）。可視性（`internal`）ではなく ArchUnit を採る理由は [ADR-0010](docs/adr/0010-confine-aggregate-creation-to-domain-service.md) を参照
