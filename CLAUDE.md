...
- 代わりに **`.claude/settings.local.json` の `sandbox.enableWeakerNetworkIsolation: true`** を有効化し、sandbox 内でシステムの TLS 信頼サービス（`com.apple.trustd.agent`）へのアクセスを許可することで、sandbox 下でも `gh` が TLS エラーなしで動くようにしている（macOS 限定オプション。ネットワーク分離はその分弱まるため、必要な範囲に留める）。
