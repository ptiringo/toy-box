...
- `@McpTool` を持つ Spring Bean は `mcp` に置く（application 層に直付けしない）。ArchUnit は `ArchSupport.kt` の `adapter("mcp", MCP)` で `mcp` を adapter リングとして強制する（[ADR-0035](../../docs/adr/0035-mcp-interface-adapter.md)）。MCP アダプタは **`local` プロファイル限定**で、`Actor` は JWT ではなく設定の `toy-box.mcp.subject-id` とツール引数の `worldId` から `McpActorFactory` が組む（[ADR-0073](../../docs/adr/0073-mcp-adapter-local-only-world-scoped.md)）。**MCP ツールは `accountId` を引数に取らない**。`Actor` は必ず `McpActorFactory` 経由で得ること（ツール引数から受け取ってよいのは `worldId` まで）。`accountId` を呼び出し側が指定できるようにすると、認証の無い MCP で他人の世界を読めてしまう。ユースケースの `Actor` 第 1 引数（detekt `ActorScopedUseCase`）やポートの `WorldId` 第 1 引数（detekt `WorldScopedPortSignature`）と同種の不変条件だが、こちらに対応する機械強制はまだ無く、**現状はレビューで担保する**（後述「機械強制しない規約（レビューで担保）」と同じ位置づけ。ArchUnit / detekt ルールの追加は別 Issue 送り）
- **MCP ツールの引数には `@McpToolParam(description = …)` で日本語の説明を付ける**（#758）。説明が無いと LLM に届く入力スキーマが引数名と型だけになり、値の出どころが推測になる。とくに**他ツールの返り値から得る ID は、どのツールが返すものかまで書く**（例: `get_jockey` の `worldId` は「`list_worlds` が返す id」）。抜けても配線は壊れず静かに劣化するだけなので、`McpServerWiringTest`（`公開する全MCPツールの引数に説明が付いている`）が**アノテーションの有無ではなく公開される入力スキーマ側**で検査する。ツールを増やすときはこのテストが自動で対象に含める
...
- **楽観ロックの競合を例外で受け取らない**（#739 / #867）。Spring Data JDBC の `save` は競合を `OptimisticLockingFailureException` で知らせるが、`SimpleJdbcRepository.save` 自身が `@Transactional` を持ちユースケースのトランザクションに**参加**するため、内側で例外が起きた時点で global rollback-only がマークされる。捕まえて `Err(UpdateConflict)` に写しても**外側のコミットが `UnexpectedRollbackException` になる**（＝409 のつもりが 500）。UNIQUE 違反を `ON CONFLICT DO NOTHING` で例外にしないのと同じ理由。update 経路（`version` が非 null）では `infrastructure.shared` の `lockRowIfVersionMatches` で対象行を `FOR UPDATE` ロックして版を突き合わせ、一致したときだけ `save` を通す（`JdbcWorldRepository` は更新行数で判定する同型）。ロックは**呼び出し側のトランザクション内で取ること**が前提で、境界の外だと防御が無症状で消えるためヘルパーが `check` で落とす。この誤りは書いた時点では緑になる（リポジトリを直呼びする契約テストでは外側にコミットすべきトランザクションが無く穴が現れない）ため、`infrastructure..` の `catch` を **detekt カスタムルール** `NoCatchOptimisticLockingFailure`（`:detekt-rules`）で検出してビルドを失敗させる（#890）。判定はソース上の型名の末尾一致で、完全修飾表記とサブタイプ（`ObjectOptimisticLockingFailureException`）を拾う。既知の限界: `DataAccessException` や `Exception` のような広い型で受ける書き方は検出できない（レビュー担保）。
