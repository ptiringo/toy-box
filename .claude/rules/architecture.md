...
- **集約をまたぐ前提条件を持つ生成口は、対応するドメインサービスからのみ呼べるよう封じ込める**（[ADR-0010](../../docs/adr/0010-confine-aggregate-creation-to-domain-service.md)）。生成口（`BloodHorse.of` / `BloodHorse.ofImported` / `BreedingRegistration.of` / `BreedingResult.of`）は `public` のまま保ち、封じ込めは可視性（`internal`）ではなく **ArchUnit** で強制する。`〜.service.horse` / `〜.service.breeding` の外から当該生成口を呼ぶことを違反とし、検証（registerInStudBook 等）を迂回した直接生成を禁止する。生成口は引数に inline value class（ID 等）を取るため JVM メソッド名が `of-Havw-KM` のようにマングルされる点を、ハッシュ手前のベース名で突き合わせて吸収する。テストは `ImportOption.DoNotIncludeTests` で対象外のため、Object Mother（`〜Fixture`）からの呼び出しは縛られない。可視性ではなく ArchUnit を選んだ経緯（`internal` の過剰許可・ソースセット結合・testFixtures 不採用）は ADR-0010 を参照
