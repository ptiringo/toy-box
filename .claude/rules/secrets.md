...
**現状、リポジトリ管理下で定義しているシークレットはない**（`fnox.toml` の `[secrets]` は空）。GitHub 操作は `gh` CLI で行う方針（[ADR-0001](../../docs/adr/0001-drop-github-mcp-use-gh-cli.md)）のため fnox での PAT 管理は不要。本ファイルは、将来 GCP 認証情報など別シークレットを扱う際の**運用規約**として維持する。
...
> **GitHub 操作は対象外**: GitHub の issue / PR 操作は `gh` CLI で行う方針（[ADR-0001](../../docs/adr/0001-drop-github-mcp-use-gh-cli.md)）のため、PAT も env 補間も `fnox exec -- claude` も不要。
