 ...
 * 記録と業務書き込みは同じトランザクションに乗るため、「キーはあるがジョッキーが居ない」状態は基本的に生じない
 * 想定。ただし記録だけが残り集約が別経路で消える（記録を連動して消さない削除等）ケースまでは防いでいないため、 `findById`
 * が引けなかった場合は失敗にせず**再登録**する。この場合、冪等キーは「同じリソースを指す」保証を失う （記録先の `resourceId` が新しく作られた集約へ上書きされる）。
 *
