...
// findOwnedByOrNotFound）を意図的に同居させている（KDoc の通り、両者は対で使う一体の口）。ファイル名はどちらの宣言名とも
...
sealed interface WorldMutationError {
     ...
     */
    data class NotFound(val worldId: WorldId) : WorldMutationError
 ...
 * を直接呼ぶ代わりに常にこちらを使う。ポート本体 [WorldRepository.findOwnedBy] と同名同引数にすると拡張関数が自分自身を
 * 再帰呼び出ししているように読めてしまう（実際にはメンバ関数が優先されポート本体を呼ぶ）ため、`findOwnedByOrNotFound` という別名にして紛らわしさを断つ。
 */
internal fun WorldRepository.findOwnedByOrNotFound(
    accountId: AccountId,
    worldId: WorldId,
): Result<World, WorldMutationError> {
    val world = findOwnedBy(accountId, worldId)
