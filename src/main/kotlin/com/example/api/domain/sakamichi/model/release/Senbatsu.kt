...
/**
 * 選抜。作品（シングル表題曲・アルバムリード曲）を歌う選ばれたメンバーの集合と、そのフォーメーション（立ち位置の割り当て）。
 *
 * 選抜はグループの恒久属性ではなく作品単位（シングル/アルバム共通）の一時的編成であり（sakamichi-sources §4）、各作品集約（`Single` / `Album`）が VO
 * として内包する。メンバーは別集約のため [MemberId] 経由の ID 参照で保持する。不変条件 （同一メンバーの重複なし・`Center` 以外の立ち位置の定員 1 人・センター 1〜2
 * 人）は生成ファクトリ [create] が検証する（ADR-0014）。
 *
