-- 集約間の ID 参照列に FK を backstop として張る（#508 / ADR-0053）。
...
-- 本ファイルは ADD CONSTRAINT ... NOT VALID（新規行への強制の即時開始）と FK 列 index まで。
-- 既存行のフルスキャン検証（VALIDATE CONSTRAINT）は ADR-0052 に従い後続の V12 へ分離する
-- （同一ファイルに同居させると ADD のロックがコミットまで残り、VALIDATE の SHARE UPDATE
-- EXCLUSIVE への格下げが効かないため）。
