...
- **allowlist へ反転する**: 壊れ方が検査漏れ側に倒れる。pre-push の glob（allowlist 的）との二重
  管理（#920）も増える。**注記（2026-09-26 訂正）**: 当初ここに「#784 と判断を揃える観点からも
  採らない」と書いたが誤りだった。#784 の `domainIsFrameworkFree` は denylist の列挙漏れが
  **違反の素通り**（危険側）になり、CI の `paths-ignore` の列挙漏れが**余計に走る**（安全側）のとは
  向きが逆である。両者で揃うのは結論ではなく「列挙漏れに気づける側を選ぶ」という原則で、その原則は
  #784 では allowlist を支持する。この ADR の決定そのもの（CI では denylist を維持する）は、上の
  2 つの理由で成立しており変わらない
