...
## 追補（2026-08-06）: 再評価トリガが引かれ、AIP-122 親子リソースを採用

再評価トリガの 1（実クライアント・実運用が現れる）と 3（AIP resource name 形式への移行を別途決める）
が、[0067](0067-per-player-world-tenant-isolation.md)（プレイヤーごとの世界によるテナント分離）で
同時に引かれた。

- **実クライアント**: フロントエンドがシミュレーションゲームとして実装され、ID を当てにする実利用者
  が現れた。
- **実在の親リソース**: `world`（セーブデータ）という実在の親リソースが生まれ、既存ドメイン
  （studbook / racing / sakamichi / tennis）の API を `/api/worlds/{worldId}/...` の親子リソース
  として構成する方針を採った（ADR-0067 の「次段階」。既存 API の実移設はまだ行っていない）。

これは「再評価時の出発点」で挙げた 2 択のうち **relative resource name（`bloodHorses/{id}`）形式**
ではなく、**URL 階層のみを親子リソース化する**という第 3 の形である。ID の外部表現そのもの
（`id: UUID` フィールドを `name: "worlds/{worldId}/bloodHorses/{id}"` のような文字列へ置き換えるか）
は本追補の対象外で、**生 UUID 据え置きは維持する**。したがって:

- パス階層（`/api/worlds/{worldId}/bloodHorses` 等）は AIP-122 に沿って親を明示するが、レスポンスの
  `id` フィールドと参照系（`〜Id`）は引き続き生 UUID のまま変えない。
- 「外部公開 ID は opaque」という規約（クライアントは構造を解釈・推測してはならない）は変更なく
  適用される。URL 階層の導入はこの規約の適用範囲を広げるものであり、破棄するものではない。
- 不透明 ID エンコード（base62 / ULID 等）への移行は、再評価トリガ 2（列挙耐性・情報漏洩が実要件に
  なる）が引かれない限り、引き続き見送る。
