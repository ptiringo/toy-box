...
> 注: 運用モデルの詳細はその後 [ADR-0040](0040-coverage-gate-operation-model.md) で更新された（ゲート対象の決め方 `includes`→`excludes` 反転 / 行下限 85%→ LINE 90% + BRANCH 80% の 2 ボーンド）。本 ADR のツール選定（Kover 採用）と「成熟領域のみゲートする」方針自体は不変。以下の `includes` / 85% の記述は当時の運用詳細で、現行は ADR-0040 を参照。
