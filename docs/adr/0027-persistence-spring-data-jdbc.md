...
- **更新系（version を進める save）はドメイン経由では未対応**。ドメインが version を持たないため、アダプタの `save` は常に insert になる。update は「version を Row 取得して引き当てる」等の方針を実装イシューで決める。
  - **解消（#424 / [ADR-0047](0047-aggregate-version-for-optimistic-locking.md)）**: 集約 version 保持 + save 一本化で解消した。共有カーネル `Entity.version`（null=未永続化）を状態遷移を持つ集約だけが override し、リポジトリポートは `findById → T?` / `save(t): Result<T, UpdateConflict>` の 2 メソッドのまま、insert / update の判別を Spring Data JDBC の version 判定に委ねる。対象は状態遷移を持つ 3 集約（BloodHorse / BreedingRegistration / BreedingResult）のみ（ADR-0041 の R/E 分類に従い、更新語彙のない Jockey と INSERT-only の HorseInspection は対象外）。
