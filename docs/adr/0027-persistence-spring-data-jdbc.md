...
- **更新系（version を進める save）はドメイン経由では未対応**。ドメインが version を持たないため、アダプタの `save` は常に insert になる。update は「version を Row 取得して引き当てる」等の方針を実装イシューで決める。
  - **解消（#424 / [ADR-0047](0047-versioned-envelope-for-optimistic-locking.md)）**: リポジトリポートに `findById → Versioned<T>?` と `update(versioned): Result<Versioned<T>, UpdateConflict>` を導入し、`save` を insert 専用に限定した。対象は状態遷移を持つ 3 集約（BloodHorse / BreedingRegistration / BreedingResult）のみ（ADR-0041 の R/E 分類に従い、更新語彙のない Jockey と INSERT-only の HorseInspection は対象外）。
