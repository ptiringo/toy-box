...
- `.squawk.toml` で除外ルールを宣言: `prefer-robust-stmts`（小規模サンドボックスで不要な `BEGIN/EXCEPTION` 要求）・`prefer-text-field`（`varchar(n)` を許容する設計判断）。`version` カラムへの `INTEGER` 型は `-- noqa: RF04` でインラインに抑制（squawk がルール ID `RF04` を誤検出する既知の問題）
...
- Python ツールだが `pipx`（+ uv）経由でインストールし、mise で `python:pipx` backend として管理することで Python 依存をツール環境内に閉じ込めることができる
...
| sqlfluff | 書式・スタイル lint・自動整形 | mise（python:pipx + uv）|
...
- **案C を採らなかった理由**: squawk のみでは書式統一を満たせない。sqlfluff の Python 依存が懸念されたが、pipx + uv で依存を閉じ込めることで他ツールと同等の管理コストに収まる。
- **引き受けるトレードオフ**:
  - sqlfluff は Python ベースのため、他の Go / Rust 系ツールと比べると初回インストールに若干時間がかかる。pipx + uv で最小化している。
